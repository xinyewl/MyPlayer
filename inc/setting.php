...
function poi_player_options() {
    add_menu_page('MyPlayer', 'MyPlayer', 'manage_options', __FILE__, 'poi_player_options_code');
}
    ...
    <div class="poi-player-center">
      ...
      <form method="post" action="options.php">
        ...
        $type = isset($_POST['select'])?$_POST['select']:''; 
          ...
          <input type="text" name="poi_options[searchnum]" class="searchnum" value="<?php if($poi['searchnum'] < 100){echo $poi['searchnum'];}elseif($poi['searchnum'] > 100){echo '100';}else{echo '20';} ?>" size="2"><span> 歌曲搜索数量（最大值为100）</span>
	      </div>
        <input type="submit" name="save" class="button" value="保存设置" />
      </form>
      <?php if ( isset($_REQUEST['settings-updated']) ){
        echo '<div id="message" class="updated"><p>保存成功！</p></div>';
      }?>
      <hr/>
      <div class="poi-ps">
      <p>歌单：http://music.163.com/#/playlist?id=<code>741361926</code>（数字即为歌单ID）</p>
      <p>专辑：http://music.163.com/#/album?id=<code>2342012</code>（数字即为专辑ID）</p>
      <p>以上两种作为类型的识别链接，自行到网易云音乐挑选歌单或专辑</p>
      <p>关于音乐搜索功能：需要主机开启fsockopen()函数，否则无法获取数据，另外可能存在主机IP被墙而无法播放歌曲的问题。</p>
	  <p>使用该功能在新建页面添加短代码<code>[MyPlayer]</code>即可。</p>
      <hr>
      <p>MyPlayer 版本<?php echo POI_VERSION; ?> 最后更新于2017.05.27 @ <a href="https://www.ikxin.com/" target="_blank">夜色静好</a></p>
      </div>
    </div>
  </div>
<?php
